...
# Enhanced RTC Configuration with multi-vendor STUN/ICE failover
...
st.markdown("Cloud-deployed suite with advanced firewall bypass and session management.")
...
if ctx.video_processor:
    # Use a unique session-based iteration counter to prevent ID collisions
    ...
    while ctx.state.playing:
        ...
        it = st.session_state.chart_iteration % 100 # Use a larger modulo to avoid reuse
        ...
        gauge_placeholder.plotly_chart(fig_gauge, use_container_width=True, key=f"gauge_chart_{it}")
        ...
        if z_data:
            ...
            bar_chart_placeholder.plotly_chart(fig_bar, use_container_width=True, key=f"bar_chart_{it}")
        ...
        if len(history) > 1:
            ...
            timeline_placeholder.plotly_chart(fig_line, use_container_width=True, key=f"line_chart_{it}")
